...
# Usage		: bash bit09-trimmomatic-PE.sh /home/user/sra/ /home/user/trimmomatic/ 
...
function usage(){
	errorString="Running this Trimmomatic script requires 4 parameters:\n
        ...
        Run on Paired End (PE) read files (*_1.fastq.gz and *_2.fastq.gz).\n
...
# Part of input/file name found with "find $1 -name '*_1.fastq.gz'"
tmpPart='"{}"';
tmpPart2='${IN}';
tmpPart3='$(basename ${IN})';
# Compose command: find files, pipe to threads and use IN for Trimmomatic
trimCommand="find $inputFolder -name '*_1.fastq.gz' | xargs -P 1";
trimCommand="$trimCommand -I {} sh -c 'IN=$tmpPart;";
# Add java call, first part of Trimmomatic command
trimCommand="$trimCommand java -jar ${pathTrimmomatic} PE -threads $4 -phred33";
# Add log file
trimCommand="$trimCommand -trimlog ${outFolder}/${tmpPart3}_log.txt";
# Add sample to process
trimCommand="$trimCommand -basein ${tmpPart2} -baseout ${outFolder}/${tmpPart3}";
